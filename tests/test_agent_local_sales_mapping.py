...
def test_build_xd_salesdocuments_query_adds_family_and_payment_mapping() -> None:
    ...
    assert "DATE(COALESCE(v.CloseDate, v.CreationDate)) AS data" in query
...
def test_build_xd_documents_query_uses_close_date_for_report_date() -> None:
    columns = {
        "DocumentKeyId",
        "ItemKeyId",
        "ItemDescription",
    ...
    assert "DATE(COALESCE(b.CloseDate, b.CreationDate)) AS data" in query
